...
def get_money(points):
	...
	for i in points_range:
		if i == points_end:
			money.append(money_line(points[-55:]))
		elif i >= 54:
			money.append(money_line(points[i-54:i+1]))
		else:
...
def logic_block(candle):
	...
	_trend = int(candle.trend)
	_close = float(candle.close)
	_open = float(candle.open)
	_low = float(candle.low)
	_zscore = float(candle.zscore)
	_money = float(candle.wema)
	#_devhigh = float(candle.dh)
	_mid = float(candle.mid)
	_mad = float(candle.mad)
	_valid = int(candle.valid)
	_strength = int(candle.strength)
	buy_logic = (
		_valid == 1,
		_trend >= -2,
		0.3819661 > _zscore > -0.3819661,
		_mad >= 0.1180339,
		_close > _money > _mid
		) # buy_logic
	sell_logic = (
...
class ThreeBlindMice:
	...
	def stats(self):
		...
		s = datetime.strptime(t[0], '%Y-%m-%d %H:%M')
		e = datetime.strptime(t[-1], '%Y-%m-%d %H:%M')
		...
		stats = {
			'sample start': t[0],
			'sample end': t[-1],
			'sample years': round(sd(int((e - s).days), 365.25), 2),
