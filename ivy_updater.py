...
__copyright__ = 'Copyright 2019, Daniel Ward'
__license__ = 'MIT'
__version__ = '1.1.1'
__codename__ = 'cria'
...
class Candelabrum:
	...
	def alpaca_candles(self, symbol, timeframe, after=None, limit=1000):
		...
		ivy.dropna(inplace=True) # axis='index', how='all', 
		...
		ivy['dl'] = dl
		ivy['mid'] = mid
		first_wema = ivy['wema'].to_numpy().nonzero()[0]
		fwi = ivy.index[first_wema[0]]
		fw = float(ivy['wema'][fwi])
		profit_check = lambda w: 1 if 0 != fw < w != 0 else 0
		ivy['valid'] = ivy['wema'].apply(profit_check)
		MAD = lambda x: numpy.fabs(x - x.mean()).mean()
		ivy['mad'] = ivy['zscore'].rolling(window=8).apply(MAD, raw=True)
		ivy['trend'] = icy.get_trend(ivy['high'].tolist(), ivy['low'].tolist())
		if self._VERBOSE:
			print(f'Alpaca candles for {symbol} took {self._TIMER.update[0]} to fetch.')
		return ivy.copy()
	...
	def update_candles(self,
		...
		if local_data is None:
			...
			_old = local_data
		#_old.drop(_old.tail(1).index, inplace=True)
	...
	def do_update(self, symbols, timeframe):
		...
		for symbol in symbols:
			...
			#print('pre-merge:', len(merge), do_merge, after)
			if len(merge) == 0: merge = None
			since, elapsed = tk.update
			ivy = self.alpaca_candles(symbol, timeframe, after=after)
			if ivy is None:
				if verbose:
					print(f'{prefix} alpaca_candles returned None {blank}', end=chr(13))
			else:
				if do_merge:
					ivy = self.update_candles(symbol, timeframe, ivy, local_data=merge)
					#print(f'post-merge: {len(ivy)}')
	...
	def cartography(self,
		...
		#title_string = f'Historical Data for {symbol} : {timeframe}'
		#ax1.set_title(title_string, fontweight='bold')
...
def cheese_wheel(silent=True,
	...
	mice = icy.ThreeBlindMice(ivy_ndx, day_trade=True, max_days=max_days)
	...
	if not silent:
		print(f'Starting quest for the ALL CHEESE using {len(ivy_ndx)} symbols.')
	tk = icy.TimeKeeper()
	...
	if not silent:
		...
		print(f'\nAfter {tk.update[0]} the quest for the ALL CHEESE comes to an end.\n')
	return (status, mice)
...
def spin_wheel():
	...
	try:
		...
		while spinning:
			ts = int(time.time())
			check_day = str(get_day(localtime()))
			if check_day != today:
				today = str(check_day)
				schedule = list(get_schedule(today, freq='5min'))
				print(check_day, len(schedule), schedule[0], schedule[-1], ts)
			if ts in schedule:
				total_spins += 1
				print(f'Spin: {total_spins}')
				s, mice = cheese_wheel(silent=False, max_days=8, chart_size=300)
				status = s[0]
				if mice:
